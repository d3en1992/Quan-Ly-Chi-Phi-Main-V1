...
| **V2 Firestore mirror (Subcollection)** | `sync.v2format.js` ghi V2 sau **manual sync** (không chạy khi auto-debounce 30s — bảo vệ free tier quota). Cấu trúc: parent document chứa summary fields (tiếng Việt, typed) + subcollection `ban_ghi/` chứa từng record riêng lẻ. V2 chỉ là mirror — V1 vẫn là source of truth cho pull/merge. Chiến lược Incremental Push: lần đầu ghi toàn bộ (full write), lần sau chỉ ghi record có `updatedAt > lastPush` và xóa record có `deletedAt > lastPush`. Timestamp `lastPush` lưu trong localStorage key `_v2SubcollLastPush`. `danh_muc` và `hop_dong` dùng full-rewrite (xóa IDs cũ + ghi lại). |
...
| `js/sync/sync.v2format.js` | `_V2_YEAR_TYPES`, `_V2_META_TYPES`, `_V2_YEAR_KEY_MAP`, `_V2_FIELD_MAPS`, `_V2_SUBCOLL_NAME`, `_V2_LAST_PUSH_KEY` | **Formatters:** `_v2FmtMoney(n)`, `_v2FmtDateTime(ts)`, `_v2FmtCtList(records,ctField)`, `_v2TypeLabel(type)` — **Converters:** `_v2ToFsValue(v)`, `_v2ToFsFields(obj)`, `_v2ApplyFieldMap(rec,map,exclude)` — **REST helpers:** `_v2ResourceBase()`, `_v2FsPatchDoc(path,fields)`, `_v2FsListIds(parentId,coll)`, `_v2FsBatchWrite(writes[])` — **Push:** `_v2GetLastPush(id)`, `_v2SetLastPush(id,ts)`, `_v2ResetLastPush(id)`, `_v2ResetAllLastPush()`, `_v2PushSubcoll(parentId,records,map,summary)`, `_v2PushSubcollFull(parentId,records,map,summary,idField?)`, `_v2PushYear(yr)`, `_v2PushMeta()` — **Debug:** `debugV2(filter?)` |
